...
function init_question(){
    for (var i = 0; i < project.questionpage_id_list.length; i++) {
        ...
        for (var k = 0; k < page.question_id_list.length; k++) {
            ...
            if (isNotEmpty(question.jumpconstraint_id_list) || isNotEmpty(question.displayconstraint_id_list)){
                set_jumpconstraint_status(get_oid(question), true, question.jumpconstraint_id_list.length + question.displayconstraint_id_list.length);
            }else{
    ...
    if (question_type != "page"){
        ...
        if (isNotEmpty(init_question)){
            ...
            syncPost(url, data, function(ret){
                ...
                if (ret.unpay_qs_num) {
                    showUnlockBtn(true, ret.unpay_qs_num);
                }
