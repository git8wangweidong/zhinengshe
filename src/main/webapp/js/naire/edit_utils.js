...
function set_jumpconstraint_status(oid, is_open, count) {
    if (is_open) {
        ...
        if(showTable.length > 0) {
            ...
            var con = '<div class="logic_sign BubR cq_lj_ts"><i class="link-icon-active"></i><span class="logic_num">' + count + '</span></div>';
        ...
        $('.module[oid=' + oid + ']').find('.showTable').remove();
    }
}
...
function Qimg_edit() {
    ...
    Qimg_obj_parent.on("click",".img_handle .edit_del" ,function(e){
        ...
        delete_option(active_oid, active_option_id);
    });
}
