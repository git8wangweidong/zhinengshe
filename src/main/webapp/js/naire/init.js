...
if (isNotEmpty(project)) {
    ...
    if (project.hasOwnProperty("displayconstraint_list")) {
        for (var i = 0; i < project.displayconstraint_list.length; i++) {
            ...
            DISPLAYCONSTRIANT_DICT[get_oid(displayconstraint)] = displayconstraint;
            var display_list = DISPLAYCONSTRAINT_SOURCE_Q_DICT[displayconstraint.question_id];
            if (!isNotEmpty(display_list)) {
                display_list = [];
            }
            display_list.push(displayconstraint);
            DISPLAYCONSTRAINT_SOURCE_Q_DICT[displayconstraint.question_id] = display_list;
        }
    }
