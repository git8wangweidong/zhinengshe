...
var pluginCenter = {
    init: function(options) {
        ...
        if (pluginName == 'envelope') pluginName = 'lucky_money';
        ...
        pluginCenter.showApplication = function() {
            ...
            function showApplicationCreate() {
                ...
                } else if (settings.type == "envelope") {
                ...
                var showApplicationStr = '' +
                    ...
                    '</dd>' +
                    '</dl>' +
                    '<a class="btn_default btn_use" href="javascript:;">立即使用</a>' +
                    '</div>' +
                    '<div class="content">' +
                    '<div class="img_wrap">' +
        ...
        pluginCenter.appPaid = function(options) {
            ...
            var tmp = '<div class="hide_logo_paid">' +
                ...
                '<h4>请选择购买类型</h4><div class="lucky_draw_paid_box"><div class="app_youhui"></div></div><div class="buy_fitler">' +
                buy_time_str +
                discount_str +
                '</div></div>' +
                '<div class="btn_wrap">' +
                ...
                '<a class="btn_prev btn_none" href="javascript:;">' + paidSettings.prevText + '</a>' +
                '</div>' +
                '</div>';
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(goodsList, function(index) {
                        var goodsListTmp = '' +
                            '<div data-scope="' + goodsList[index].use_scope + '" id="' + goodsList[index].oid + '" class="amount_info">' +
        ...
        pluginCenter.limiteMobileComplete = function(pid, is_modify) {
            ...
            var $btnSmsSwitch = $limiteMobileComplete.find('.btn_sms_switch');
            ...
            if (SMSRemainderNum == 0) {
                $btnSmsSwitch.addClass('btn_disabled');
            ...
            $btnSmsSwitch.click(function() {
                ...
                if ($this.hasClass('open')) {
                    $this.removeClass('open').siblings('.state_txt').text('已关闭').removeClass('blue');
                    $.ajax({
                        url: '/plugin/ajax/update_setting/',
                        'type': 'POST',
                        data: {
                    ...
                    $this.addClass('open').siblings('.state_txt').text('已开启').addClass('blue');
                    $.ajax({
                        url: '/plugin/ajax/update_setting/',
                        'type': 'POST',
                        data: {
        ...
        pluginCenter.showBuyProjectList = function(expire_time, is_expire) {
            ...
            if (settings.type == 'lucky_draw' || settings.type == 'restrict_mobile_rspd') {
                ...
                if (expire_time && typeof(expire_time) != "undefined") {
                    ...
                    if (is_expire) {
                        buyAgain = '应用已于' + expire_time.toString() + '到期，<a class="btn_again_buy" href="javascript:;">[立即续费]</a>重新开启抽奖应用';
        ...
        pluginCenter.envelopePaid = function(options) {
            ...
            $envelopePaid.on('click', '.btn_next', function() {
                _hmt.push(['_trackEvent', 'pluginAliPay', 'click', 'lucky_money_pluginAliPay']);
                if (getAvailableRmb().status == '200') {
                    var leave_money = parseFloat(availableRmb);
        ...
        pluginCenter.hideLogoRenew = function(expire_time) {
            ...
            if (nowdate > enddate) {
                overdueStr = '<h6 class="f16">已于 ' + expire_time + ' 到期，如需使用请 <a href="javascript:;" class="renew_btn_v2">再次购买</a></h6><p>到期后问卷网标志将再次显示</p>';
            } else {
                overdueStr = '<h6>该功能有效期至<span>' + expire_time + '</span></h6><p>账号下所有项目均已隐藏问卷网标志，打开您的问卷、表单、测评试试吧</p><a href="javascript:;" class="renew_btn">续费延期</a>';
            }
        ...
        pluginCenter.historyRecordList = function(pid) {
            ...
            if (settings.type == 'envelope') {
                for (var i = 0; i < his_datas.datas.length; i++) {
                    var status_str = '';
                    if (i == 0 && his_datas.datas[i].status == 3 || his_datas.datas[i].status == 4 || his_datas.datas[i].status == 5) {
        ...
        pluginCenter.historyRecordWinnerList = function(batch_id, pid, chart_datas_old, table_datas_old) {
            ...
            var tmp = '<div class="history_record history_record_winner_list">' +
                ...
                '<ul class="page_ck"><li class="input_ck"><i></i>全选</li><li class="status_bt" st="2">待发放</li><li class="status_bt" st="3">已发放</li><li class="status_bt" st="1">不发放</li></ul>' +
            ...
            var status_bt_obj = $historyRecordWinnerList.find($('.history_record .page_ck li.status_bt'));
            var  ck_bt_obj = $historyRecordWinnerList.find("i.ck_bt");
            if (ck_bt_obj.length == 0){
                $historyRecordWinnerList.find(".page_ck").hide();
            }
            $historyRecordWinnerList.on('click', 'i.ck_bt', function() {
                $(this).toggleClass('active');
                if ($historyRecordWinnerList.find("td i.active").length == 0){
                    status_bt_obj.removeClass('status_bt_show').removeClass('active');
                }else{
                    status_bt_obj.addClass('status_bt_show');
                }
            });

            $historyRecordWinnerList.on('click', '.page_ck .input_ck', function() {
                if ($(this).find('i').hasClass('active')){
                    $(this).find('i').removeClass('active')
                    $('.history_record .history_list_table tr i').removeClass('active');
                    status_bt_obj.removeClass('status_bt_show').removeClass('active');
                }else{
                    $(this).find('i').addClass('active')
        ...
        function commonComplete(options) {
            ...
            var tmp = '<div class="common_complete">' + imgTmp + h4TextTmp + textTmp + btnTmp + envelope_info + '</div>';
            pluginCenter.showChildWrap(settings.info.title, tmp);
            ...
            $('.head_title').on('click', '.close', function() {
                if (settings.type == 'envelope' || settings.type == 'lucky_draw') {
